...
5. What is a good "Pyre" way to organize components based on physics?
...
7. Review creation of Subfield classes in petsc/subfields/__init__.py.

8. What is the best way to provide defaults for a component based on its family name? What is required for this to work? Does `import pylith` need to provide `pylith.governing_eqns.elasticity` or can it be imported using`import pylith.governing_eqns.elasticity`?
...
9. Is naming a component supposed to override settings via the hierarchy:
...
10. `mm` does not appear to be building the Pyre mpi package. I have MPI installed in `/software/baagaard/openmpi-5.0/gcc-11.4/`.
